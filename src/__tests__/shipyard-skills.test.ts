...
describe('shipyard skills — behavior & packaging contract', () => {
  ...
  it('harbor sheet language: English by default, never the chat session language', () => {
    // Regression: a live sweep once posted Chinese sheets into an English
    // repository because it followed the conversation language instead of
    // harbor's output contract.
    expect(HARBOR).toContain('are written in English');
    expect(HARBOR).toContain('English is the default language of record for harbor output');
  ...
  it('harbor survives sloppy and drive-by PRs (no-claim and proportionality rules)', () => {
    ...
    expect(HARBOR).toContain('does not guess intent from the diff');
